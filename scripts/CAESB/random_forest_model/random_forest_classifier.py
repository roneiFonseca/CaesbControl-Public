...
# Criar e treinar o modelo Random Forest com GridSearchCV
...
with open(os.path.join(script_dir, "random_forest_results.txt"), "w") as f:
    f.write("--- Resultados do Modelo Random Forest ---\n\n")
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, "random_forest_results.png"), dpi=300, bbox_inches='tight')
plt.show()

print("\nResultados salvos em:")
print(f"- {os.path.join(script_dir, 'random_forest_results.txt')}")
print(f"- {os.path.join(script_dir, 'random_forest_results.png')}")

# Versão simplificada do modelo (opcional)
print("\n--- Criando versão simplificada do modelo ---")
# Reduzir o número de parâmetros para o Grid Search para uma execução mais rápida
...
simplified_grid_search.fit(X_train_scaled, y_train)
simplified_best_params = simplified_grid_search.best_params_
print(f"Melhores parâmetros simplificados: {simplified_best_params}")

# Avaliar o modelo simplificado
simplified_model = RandomForestClassifier(random_state=42, **simplified_best_params)
simplified_model.fit(X_train_scaled, y_train)
simplified_y_pred = simplified_model.predict(X_test_scaled)
simplified_f1 = f1_score(y_test, simplified_y_pred, zero_division=0)
print(f"F1-Score do modelo simplificado: {simplified_f1:.4f}")
